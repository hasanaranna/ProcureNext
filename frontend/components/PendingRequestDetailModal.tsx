    ...
    <div className="flex items-center justify-between py-3 border-b border-slate-100 last:border-0">
      ...
      {filename ? (
        <a href={filename} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-xs font-semibold text-accent-600 bg-accent-50 border border-accent-200 px-3 py-1 rounded-full hover:bg-accent-100 transition-colors">
...
export default function PendingRequestDetailModal({
  ...
  return (
    <ModalShell isOpen={isOpen} onClose={onClose} maxWidth="max-w-2xl">
      {/* Header */}
      <div className="bg-gradient-to-r from-navy-950 to-navy-900 px-6 py-5 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-accent-500 to-accent-600 flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-lg">
            {registration.name.charAt(0)}
          </div>
          <div>
            <h2 className="text-lg font-bold text-white leading-tight">{registration.name}</h2>
            <p className="text-sm text-slate-400">{registration.company}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="overflow-y-auto flex-1 px-6 py-6 space-y-6">
          ...
          <div className="bg-slate-50 rounded-xl border border-slate-200 overflow-hidden">
            <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-200">
              <div className="px-5 py-4">
                ...
                <p className="text-sm font-bold text-navy-900">{registration.name}</p>
              </div>
              <div className="px-5 py-4">
                <p className="text-xs text-slate-400 font-medium mb-1">Phone Number</p>
                <p className="text-sm font-bold text-navy-900">{registration.phone}</p>
              </div>
            </div>
            <div className="border-t border-slate-200 px-5 py-4">
              <p className="text-xs text-slate-400 font-medium mb-1">Email Address</p>
              <p className="text-sm font-bold text-navy-900">{registration.email}</p>
            </div>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="bg-slate-50 rounded-xl border border-slate-200 px-5 py-4">
            <p className="text-xs text-slate-400 font-medium mb-1">Organization Name</p>
            <p className="text-sm font-bold text-navy-900">{registration.company}</p>
          </div>
        </section>

        {/* Identity Documents */}
        <section>
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Identity Documents</h3>
          <div className="bg-slate-50 rounded-xl border border-slate-200 px-5 py-1">
        ...
        <section>
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Regulatory Documents</h3>
          <div className="bg-slate-50 rounded-xl border border-slate-200 px-5 py-1">
        ...
        <section>
          ...
          {registration.documents.additionalDocs.length > 0 ? (
            ...
            <p className="text-sm text-slate-400 italic bg-slate-50 rounded-xl border border-slate-200 px-5 py-4">
      ...
      <div className="flex-shrink-0 px-6 py-4 border-t border-slate-200 bg-white">
        <div className="flex items-center justify-end gap-3">
          <button onClick={() => handleAction(onDecline)} disabled={isSubmitting}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-red-500 hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl transition-all shadow-sm">
            {isSubmitting && (
          ...
          <button onClick={() => handleAction(onAccept)} disabled={isSubmitting}
            className="flex items-center justify-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-bold rounded-xl transition-all shadow-lg">
            {isSubmitting && (
              <svg className="animate-spin w-4 h-4 text-white" viewBox="0 0 24 24" fill="none">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
